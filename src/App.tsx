        ...
        {state.users.length > 0 && !state.connecting && (
          <>
            ...
            <Box sx={{marginTop: 5}}>
              ...
              {(state.isReady && (state.allUsersReady || fin)) && (
                <>
              ...
              <Stack direction="row" spacing={2} useFlexGap flexWrap="wrap" justifyContent="center">
                {!fin && bothSelected && !ready && <Button color="success" variant="contained" fullWidth={true} onClick={onImReadyClicked}>I'm ready!</Button>}
                {state.imHost && <Button variant="outlined" onClick={onNewEstimateClicked}>New estimate</Button>}
                {state.imHost && !fin && !state.allUsersReady && <Button variant="outlined" onClick={onShowResultsClicked}>Show results</Button>}
              </Stack>
            </Box>
          </>
        )}
